...
def get_states_prop_holds(tcgs: TimedCGS, prop: str) -> Optional[Set[str]]:
    """Return set of locations where an atomic proposition holds.

    The AE version accepts three convenient cases:
    1. standard atomic propositions from the model labelling matrix;
    2. direct use of state names as propositions;
    3. a few compatibility aliases used in the paper examples.
    """
    aliases = {
        "takeCash": "cash",
    ...
    # Direct state-name propositions
    if prop in set(tcgs.get_states()):
        return {prop}
...
def _map_formula_clock(tcgs: TimedCGS, clock: str) -> str:
    """Map formula freeze clocks j, j1, j2, ... to automaton global clock y when available."""
    if isinstance(clock, str) and clock.startswith("j") and "y" in tcgs.clocks_dict:
        return "y"
    return clock
